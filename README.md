...
- 空闲 20 秒背光调暗,45 秒熄灭;按键会点亮屏幕,这次按键照常生效。
...
| 主机测试 | 固件 `validate.sh --static` 通过(含 `pk_protocol` 测试与受保护分区布局检查);Mac app 18 个单元测试通过 |
...
| 未验证 | 拔线后屏幕状态切换的具体耗时;背光调暗与熄灭时序;登录时自动启动;全新 Mac 上的首次蓝牙授权流程;长时间运行的耗电 |
