...
| 背光 | 活动时 80%,空闲 20 秒调到 10%,45 秒熄灭。按键或连接变化会点亮;唤醒屏幕的那次按键照常转发。 |
...
Mac 每 5 秒发送一次 ping。设备在 15 秒收不到命令、收到 `bye`、BLE 断开或 USB 拔线时认为链路离线。`(boot, seq)` 供 Mac 丢弃重复的按键。
